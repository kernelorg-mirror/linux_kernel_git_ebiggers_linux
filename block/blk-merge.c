 ...
 *
 * Condition 2) *If possible*, while still satisfying Condition 1, we'd like to
 * have start_offset + @return divisible by the physical block size in sectors.
 */
				       ...
				       struct bio *bio)
{
	...
	unsigned int alignment = bio_required_sector_alignment(bio);
	...
	if (pbs_aligned_sector > start_offset &&
	    IS_ALIGNED(pbs_aligned_sector - start_offset, alignment))
		return pbs_aligned_sector - start_offset;

	return round_down(sectors, alignment);
}
