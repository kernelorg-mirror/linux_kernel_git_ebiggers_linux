...
{
	aes_gcm_encrypt(data, data, data_len, authtag, NULL, 0,
...
static int ceph_aes_gcm_decrypt(u8 *data, const u8 *authtag, size_t data_len,
				struct ceph_connection *con)
{
	int err = aes_gcm_decrypt(data, data, data_len, authtag, NULL, 0,
				  ...
				  struct ceph_msg *msg)
{
	...
	if (front || middle) {
		...
		if (middle) {
			memcpy(&bounce[front + front_pad],
			       msg->middle->vec.iov_base, middle);
			memset(&bounce[front + front_pad + middle], 0,
			       middle_pad);
		}
